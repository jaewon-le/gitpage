...
- 라이브러리는 프로그래머들이 이미 만들어진 코드를 다시 개발하지 않아도 되게 하고 서로 함께 작업할 수 있도록 만들어주는 함수의 모음입니다. 라이브러리는 문자를 대문자에서 소문자로 변환하거나, 문자열을 정수로 변환하는 것 등 c에서 자주 쓰이는 연산들의 함수를 포합합니다. 여러분의 프로그램에서 비슷한 기능을 수행해야 한다면 이미 만들어져 있는 함수를 또 만든 대신 이미 존재하는 라이브러리를 그대로 사용할 수 있습니다.
...
- 데이터를 묶어 효율적으로 활용하기 위하여 배열을 사용하였습니다. 배열의 장점은 각 학생의 인덱스를 알 때, 모든 학생들에게 임의 접근할 수 있으며, 각 배열 원소를 순환하면서 확인할 수 있다는 것입니다. 하지만 배열은 같은 테이터형의 변수들을 하나로 묶을 수 있지마느 서로 다른 데이터형의 변수를 묶어서 사용할 수 없습니다. 그리고 사용 전에 배열의 크리를 선언해야만 합니다.
